...
parser = argparse.ArgumentParser(description="OPCUA node mapping services. Optionally starts a Modelica simulation in interactive mode via OMPython package.")
parser.add_argument("-c","--config_path", help="Path to config.json file which defines OPCUA server information, node mapping and optionally Modelica simulation setup.", type=str, default="config.json")
...
if plcclient:
    ...
    mapper = OPCUAFMUMapper(config=config["Mapping"], plc_client=None, mappables=component_list)
    mapper.simulate()
