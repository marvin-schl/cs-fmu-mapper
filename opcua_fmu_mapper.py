...
class OPCUAFMUMapper:
    ...
    def __init__(self, plc_client: SynchronizedPlcClient = None, mappables = [], config = None):
    ...
    def init_node_maps(self):
        ...
        for key, value in (self._config["postStepMappings"]).items():
            names.append(key)
            names = [*names, *value]
        self._name_component_map = dict(map(lambda x: (x, self.get_component_to_name(x)), names))
